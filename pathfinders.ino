...
//Change this for specific goal coordinates. [this is made for 16x16. start coordinate is 0,0 (bottom left) on a positive x and y axis]
...
void floodToFinish(unsigned char flood[N][N], unsigned char walls[N][N]){
    ...
    flood[7][7] = 1; flood[7][8] = 1; flood[8][7] = 1; flood[8][8] = 1; //change this as well to follow the coordinate of end goal (should be same as isFinish)
    enqueue(q, 7); enqueue(q, 7); enqueue(q, 7); enqueue(q, 8); enqueue(q, 8); enqueue(q, 7); enqueue(q, 8); enqueue(q, 8); // change the values of this as well, according to the coordinates of end goal.
...
void loop(){
    ...
    int x=0, y=0, turnR=0;
    unsigned char flood[13][13] = {0};
    unsigned char walls[13][13] = {0};

for (int i=0; i<3; i++){ // change this number if you want to have lesser exploration run (if mouse speed is slow.)
        while (!isFinish(x, y)) {
